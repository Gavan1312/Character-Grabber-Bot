...
async def roll_dart(client: Client, message: t.Message):
    ...
    if slot_amount < min_bet_amount:
        return await message.reply_text(
            f"Please bet at least 5% of your {currency_names_plural['balance']}, which is {currency_symbols['balance']}`{min_bet_amount:,.0f}`."
        )

    max_bet_amount = int(bal * 0.4)
    if slot_amount > max_bet_amount:
        return await message.reply_text(
            f"Can't bet more than 40% of your {currency_names_plural['balance']}, which is {currency_symbols['balance']}`{max_bet_amount:,.0f}`."
        )
    ...
    if slot_value in [1, 22, 43]:  # Triple match (Jackpot)
        ...
        await message.reply_text(f"[🎰] **JACKPOT!** 🎉\nYou won {currency_symbols['balance']}`{reward:,.0f}`!")
